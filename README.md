...
При создании ставки у игрока сразу списывается валюта, при выигрыше возвращается сумма ставки, а также сумма, умноженная на коэффициент возврата.
...
Для изменения используется функция *dunder setitem*(имя игрока, значение) - для значения учитывается знак. Все изменения баланса логгируются в файле `balance.log`
...
Единственное условие, при котором симуляция не сможет дальше идти - у всех игроков закончатся деньги. При таком исходе симуляция завершится раньше, и игроки будут считаться поверженными гусями.
